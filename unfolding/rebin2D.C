...
//double binning0[] = {0,5,10,15,20,25,30,35,40,45,50};
//double binning1[] = {0,10,20,30,40,50};
//std::vector<double> binningVec0 = {0,5,10,15,20,25,30,35,40,45,50};
...
void rebin2D()
{
 ...
 OutputEntries(hMatrixM,hMatrixRebin);
 ...
 TH2D*h3 = new TH2D("h3","",nBins0,binning0,100,0,50);
 h3->SetTitle("h3");
 ...
 TH2D*h3Rebin = Rebin2D(h3,"h3Rebin",binningVec1);
 h3Rebin->SetTitle("h3Rebin");
 ...
 h3->Draw("colz");
 TCanvas*c2Rebin = MakeCanvas("c2Rebin");
 h2Rebin->Draw("colz");
 TCanvas*c3Rebin = MakeCanvas("c3Rebin");
 h3Rebin->Draw("colz");
*/
...
TH2D*Rebin2D(TH2D*hist,TString histName,std::vector<double> binning)
{
 ...
 TH2D*hRebin = new TH2D(histName,"",nBinsHistX,massbinsTrue,nBinsReco,newbinning);
