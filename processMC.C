...
//Initialization of branch names within ntuples
...
const TString histTitles[nHistos] = {"Only Kinematic Cuts","Reco-Gen Matched",
  "Medium ID Cuts","All FSR Events: No cuts","HLT Cut","Reconstructed"};
...
void processMC()
{
 ...
 TH2F*hInvMassVsRapidity = 
   new TH2F("hInvMassVsY","",nLogBins,massbins,nRapBins,rapLow,rapHigh);
  hInvMassVsRapidity->GetXaxis()->SetTitle("Rapidity");
  hInvMassVsRapidity->GetYaxis()->SetTitle("mass [GeV]");
  hInvMassVsRapidity->GetYaxis()->SetMoreLogLabels();
  hInvMassVsRapidity->GetYaxis()->SetNoExponent();
 ...
 for(int iChain=0;iChain<numChains;iChain++)
   {
	...
	{      
	  ...
	  histInvMass[ALL_ELE]->Fill(invMassFSR,totalWeight);
          histRapidity[ALL_ELE]->Fill(rapidityFSR,totalWeight);
	  ...
	  histInvMass[KINEMATIC_CUTS]->Fill(invMassFSR,totalWeight);
          histRapidity[KINEMATIC_CUTS]->Fill(rapidityFSR,totalWeight);
	  ...
	  bool genToRecoMatchedLep1 = findGenToRecoMatch(idxGenEleFS1,closestTrackLep1);	         bool genToRecoMatchedLep2 = findGenToRecoMatch(idxGenEleFS2,closestTrackLep2);	      
	  ...
	  histInvMass[RECO_MATCHED]->Fill(invMassFSR,totalWeight);
          histRapidity[RECO_MATCHED]->Fill(rapidityFSR,totalWeight);
	  ...
	  histInvMass[ID_CUTS]->Fill(invMassFSR,totalWeight);
          histRapidity[ID_CUTS]->Fill(rapidityFSR,totalWeight);
	  
	  // Apply HLT requirement
	  if(!passHLT) continue;
	  histInvMass[HLT_CUTS]->Fill(invMassFSR,totalWeight);
          histRapidity[HLT_CUTS]->Fill(rapidityFSR,totalWeight);
	  ...
	  histInvMass[RECO_ELE]->Fill(invMassReco,totalWeight);//Should I apply SFs?
          histRapidity[RECO_ELE]->Fill(rapidityReco,totalWeight);//Should I apply SFs?
          ...
          hInvMassVsRapidity->Fill(invMassFSR,rapidityFSR,totalWeight);
	}//end event loop   
  ...
  TFile *rootFile = new TFile("./data/efficiencyAndUnfolding.root","RECREATE");
  rootFile->cd();
  for(int i=0;i<nHistos;i++){
    histInvMass[i]->Write();
...
void counter(Long64_t i, Long64_t N)
{
  ...
  if(i%(N/100)==0)
    {
      cout << "processMC.C " << "[Time: " << eventTimeStamp.AsString("l") << "] " << P << "%" << endl;
    }
  return;
}
