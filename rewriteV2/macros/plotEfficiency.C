...
TGraphAsymmErrors*setOptions(TEfficiency*eff,TPad*pad,float markSize = 0.6,int markStyle = 20);
...
void plotEfficiency()
{
 ...
 TEfficiency*HLTEfficiency = (TEfficiency*)file->Get("HLTEfficiency");
 ...
 canvas->cd();
 TPad*pad4 = new TPad("","",0.5,0,1,0.5,kWhite,0,0);
 ...
 pad4->Draw();
 pad4->cd();
 auto gHLT = setOptions(HLTEfficiency,pad4);
 gHLT->GetYaxis()->SetLabelSize(0);
 gHLT->GetXaxis()->SetTitle("mass [GeV]");
 gHLT->Draw("PE");
 midAxis->Draw("same");
